...
import { Sparkles, Wand2, MessageCircle, FileText, Scale } from 'lucide-react';
...
export const AIReviewStudio = () => {
  ...
  const bookOptions = useMemo(() => {
    const all = [...(reading.currentlyReading || []), ...(reading.booksRead || [])];
    return all;
  }, [reading.booksRead, reading.currentlyReading]);

  const [selectedBookId, setSelectedBookId] = useState(bookOptions[0]?.id || '');
  const book = useMemo(() => bookOptions.find((b) => b.id === selectedBookId), [bookOptions, selectedBookId]);
  ...
  const [loading, setLoading] = useState({ review: false, summary: false, chat: false, compare: false });
  ...
  const generateSummary = async () => {
    ...
    try {
      const out = await aiService.generateSummary(book, summaryMode);
      setSummaryOut(out || '');
    } catch (e) {
      ...
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="font-display text-5xl font-bold gradient-text">AI Review Studio</h1>
          </div>
          <p className="text-gray-400">Generate reviews, summaries, comparisons, and chat personas (mock AI).</p>
        </motion.div>
        ...
        <div className="grid gap-6">
          <GlassCard>
            {hasBooks ? (
              <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
                <div>
                  ...
                  <div className="text-sm text-gray-400">
                    Books shown here come from your library / reading lists and power all AI tools below.
                  </div>
                </div>
                <select
                ...
                >
                  <option value="" className="bg-white dark:bg-dark-800 text-slate-500 dark:text-gray-400">
                    Choose a book…
                  </option>
                  {bookOptions.map((b) => (
                    <option
              ...
              <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
                <div>
                  <div className="font-medium">No books in your library yet</div>
                  <div className="text-sm text-gray-400">
                    Add books to your reading list to unlock AI reviews, summaries, and chat for them.
                  </div>
                </div>
                <Link to="/explore">
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <GlassCard>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-display text-xl font-bold">AI Review Generator</h3>
              </div>
                ...
                {tones.map((t) => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm transition-all ${
                      tone === t.id ? 'bg-neon-magenta text-dark-900 font-medium' : 'glass text-gray-300 hover:bg-white/10'
                    }`}
                  >
                    {t.label}
                  </button>
              ...
              <Button icon={Wand2} onClick={generateReview} disabled={loading.review || !book}>
            ...
            <GlassCard>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="font-display text-xl font-bold">AI Summary Modes</h3>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {summaryModes.map((m) => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm transition-all ${
                      summaryMode === m.id ? 'bg-neon-amber text-dark-900 font-medium' : 'glass text-gray-300 hover:bg-white/10'
                    }`}
                  >
                    {m.label}
                  </button>
              ...
              <Button icon={FileText} onClick={generateSummary} disabled={loading.summary || !book}>
          ...
          <GlassCard>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-display text-xl font-bold">Chat with the Book</h3>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {personas.map((p) => (
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm transition-all ${
                    persona === p.id ? 'bg-neon-cyan text-dark-900 font-medium' : 'glass text-gray-300 hover:bg-white/10'
                  }`}
                >
                  {p.label}
                </button>
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              <div className="glass-dark rounded-xl p-4 h-72 overflow-auto">
                {chatMessages.length === 0 ? (
                  <div className="text-gray-400 text-sm">Ask something to start a conversation.</div>
                ) : (
                  <div className="space-y-3">
                    {chatMessages.map((m, idx) => (
                      <div key={idx} className={`text-sm ${m.role === 'user' ? 'text-white' : 'text-gray-300'}`}>
                        <span className={`font-medium ${m.role === 'user' ? 'text-neon-cyan' : 'text-neon-magenta'}`}>
                          {m.role === 'user' ? 'You' : 'AI'}:
                        </span>{' '}
                        {m.text}
                      </div>
              ...
              <div className="space-y-3">
                ...
                />
                <Button icon={MessageCircle} onClick={sendChat} disabled={loading.chat || !book}>
          ...
          <GlassCard>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-display text-xl font-bold">AI Review Comparison</h3>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-4">
              <div>
                ...
                <div className="mt-3 space-y-1">
                  <Button icon={Scale} onClick={compare} disabled={loading.compare || !book}>
                    {loading.compare ? 'Comparing...' : 'Compare with internet reviews'}
                  </Button>
                  {!book && (
              ...
              <div className="glass-dark rounded-xl p-4">
                {!comparison ? (
                  <div className="text-gray-400 text-sm">Run a comparison to see similarity, originality, and highlights.</div>
                ) : (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-gray-400">Similarity</span>
                      <span className="text-neon-cyan font-bold">{comparison.similarity}%</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-gray-400">Originality</span>
                      <span className="text-neon-magenta font-bold">{comparison.originalityScore}%</span>
                    </div>
                    <div className="mt-3">
                      <div className="text-sm font-medium mb-1">Unique points</div>
                    ...
                    <div className="mt-3">
                      <div className="text-sm font-medium mb-1">Common points</div>
                      <ul className="text-sm text-gray-300 list-disc pl-5 space-y-1">
                    ...
                    <div className="mt-3">
                      <div className="text-sm font-medium mb-1">Contradictions</div>
                      <ul className="text-sm text-gray-300 list-disc pl-5 space-y-1">
